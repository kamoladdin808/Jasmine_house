...
if (typeof module !== 'undefined' && module.exports) {
  ...
  const FULL_TRANSLATIONS = {
    ...
    "ЗАВТРАК ПО ТУРЕТСКЫЙ": { uz: "Turkcha nonushta", en: "Turkish breakfast" },
    ...
    "ВАФЛИ КЛУБ БАНАН": { uz: "Qulupnay-banan vafli", en: "Strawberry banana waffle" },
  ...
  function injectMenu() {
    ...
    tabs.forEach(tab => {
      ...
      menuData[tab].forEach(item => {
        menuStr += `        { name: '${esc(item.name)}', desc: '${esc(item.desc)}', weight: '${esc(item.weight)}', price: ${item.price}, badge: null, bg: '${item.bg}' },\n`;
      });
      menuStr += `      ],\n`;
    });
    menuStr += '    };';
